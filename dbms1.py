...
def addNewDB():
    ...
    tk.Label(root,text='Add New DataBase',font=('ubuntu',20)).pack()    
...
def createDB():
    try:
        ...
        if(err.errno==1007):
            ...
            mb.showinfo('Alert!','Enter a vaild Database Name')
        else:
            mb.showinfo('Alert!','Faild to Create database:{}'.format(err))
...
def createTabFun():
    try:
        ...
        print(tabs)
        if(colNum.get()==''):
            mb.showinfo('Alert!','Enter the Vaild Column Numbers')
        elif(tabName.get() in tabs):
            mb.showinfo('Alert!','{} Table Already Exist'.format(tabName.get()))
        elif(tabName.get() ==''):
            mb.showinfo('Alert!','Enter the Vaild Table Name')
        else:
            top=tk.Tk()
            top.title('Table Details')
...
def showTabFun():
    try:
        ...
        cur.execute("SELECT * FROM {} LIMIT 35".format(showTabName.get()))
        dataList=[()]
        for i in cur:
            dataList.append(i)
        if(len(dataList)==0):
...
def secureLogin():
    ...
    tk.Label(root,text='Please Provide your Credential Details').place(x=115,y=100)
...
def validDetails():
    ...
    try:
        ...
        print(err.errno)
        if err.errno==2003:
            mb.showinfo('Alert!','     Access Denied! \nEnter Valid Hostname! \ Server is offline!')
        elif err.errno==1049:
            mb.showinfo('Alert!','Database Doesnot Exist!')
        elif err.errno==1045:
            mb.showinfo('Alert!','      Access Denied!\nEnter Valid Username or \n         Password!')
        else:
            mb.showinfo('Alert!','Something Wrong!!')
